...
DEFINE_string(txt_path, "./data/ch3/imu_points_imu_sim_2points.txt", "数据文件路径");
// DEFINE_string(txt_path, "./data/ch3/imu_points_imu_sim_4points.txt", "数据文件路径");
...
int main(int argc, char** argv) {
    ...
    .SetLandmarksProcessFunc([&](const sad::Landmarks& landmarks) {
        ...
        //     // get roll, pitch, yaw
        //     Vec3d euler_before = iekf.GetOrientation().matrix().eulerAngles(2, 1, 0);
        //     LOG(INFO) << "euler_before = " << euler_before.transpose() * 180 / M_PI;

        //     SO3 R_biased = iekf.GetOrientation() * SO3(Rwb);
        //     iekf.SetOrientation(R_biased);
        //     Vec3d euler_after = iekf.GetOrientation().matrix().eulerAngles(2, 1, 0);
