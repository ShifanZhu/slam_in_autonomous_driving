...
class IMUIntegration {
   public:
    ...
    void AddIMU(const IMU& imu) {
        ...
        if (dt > 0 && dt < 0.1) {
            // 假设IMU时间间隔在0至0.1以内
            p_ = p_ + v_ * dt + 0.5 * gravity_ * dt * dt + 0.5 * (R_ * (imu.acce_ - ba_)) * dt * dt; // 3.15b
            v_ = v_ + R_ * (imu.acce_ - ba_) * dt + gravity_ * dt; // 3.15c
            R_ = R_ * Sophus::SO3d::exp((imu.gyro_ - bg_) * dt); // 3.15a Update orientation last cause p_(t+dt) needs R_(t)
        }
